    ...
    <div class="video-feed-scroll video-feed-container h-screen overflow-y-scroll snap-y snap-mandatory scrollbar-hide smooth-scroll bg-white dark:bg-gray-900" x-data="videoNavigation()">
        ...
        @empty
            <x-empty-state
                icon="video"
                title="No videos yet"
                message="{{ auth()->check() ? 'Be the first to upload a video!' : 'Login to start uploading videos!' }}"
                :actionUrl="auth()->check() ? route('videos.create') : route('login')"
                :actionText="auth()->check() ? 'Upload Video' : 'Login'"
            />
        @endforelse
        ...
        function videoNavigation() {
            return {
                scrollToNext() {
                    ...
                    if (!activeContainer) {
                        // Nếu không có video active, scroll đến video đầu tiên
